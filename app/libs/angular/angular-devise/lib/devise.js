...
(function(angular) {
    ...
    this.$get = ['$q', '$http', '$rootScope', '$cookies', function($q, $http, $rootScope, $cookies) {
        // Our shared save function, called
        // by `then`s.
        function save(user) {
            //alert(user.user.status);
        if (user == null){
            user= null;
        }
        else{

            if (typeof user.authentication_token != 'undefined'){ // this is a check to not allow devise to set  user cookies at the time of sign up
                var user_text ='{ "authentication_token":"'+user.authentication_token+'" , "organizations":"'+user.user.organizations.length +'", "email":"'+user.user.email+'", "fname":"'+user.user.fname+'", "lname":"'+user.user.lname+'", "user_type":"'+user.user.user_type+'", "status":"'+user.user.status+'"}'
                var user1 = JSON.parse(user_text);
                $cookies.putObject('user', JSON.stringify(user1));
                service._currentUser = user1;
            }
        };
        ...
        var service = {
             ...
             * on the scope.
             */
            _currentUser: $cookies.getObject('user')? JSON.parse($cookies.getObject('user')): null,
            ...
            logout: function(config) {
                ...
                return $http(httpConfig('logout', undefined, config))
                    ...
                    .then($cookies.putObject('user', null));
            },
